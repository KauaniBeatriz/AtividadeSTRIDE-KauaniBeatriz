...
router.post("/transactions/transfer", (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "falha ao processar transferência" });
  }
});
...
router.post("/validate/email", (req: Request, res: Response) => {
  ...
  // Regex simples, sem quantificadores aninhados — sem risco de ReDoS
...
router.post("/reports/generate", (req: Request, res: Response) => {
  ...
  const allowedTypes = ["summary", "balance"] as const;
  if (!allowedTypes.includes(type as (typeof allowedTypes)[number])) {
    return res.status(400).json({ error: "type inválido" });
  }

  // Geração controlada, sem chamar o shell nem interpretar código dinâmico
  const output = `stride-ok-${type}`;
